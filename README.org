...
cd ~/.emacs.d && git checkout e51391fa
#+end_src
...
#+begin_src bash
git clone --recursive https://github.com/swinkels/spacemacs-config.git ~/.spacemacs.d
#+end_src
...
The above clone command uses option ~--recursive~ to also clone [[https://github.com/swinkels/yasnippets][my YASnippet
files]] [fn:2] as a Git submodule to subdirectory ~snippets~. Spacemacs will
automatically pick them up from this directory. The use of Git submodules is a
story in itself and I'm still on the fence if an ordinary subdirectory with its
own Git repo would not be easier. When in doubt, treat ~snippets~ as any
subdirectory with its own Git repo, for example, a ~git pull~ in that directory
will retrieve my latest snippets.
