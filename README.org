...
terminal version always uses the arrow separator in the modeline, even if you
specify it otherwise. For consistency reasons I let the GUI version also use the
arrow separator.
#+BEGIN_SRC emacs-lisp :noweb-ref Frame-appearance
